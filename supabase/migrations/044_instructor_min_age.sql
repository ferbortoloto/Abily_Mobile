-- ============================================================
-- Adiciona validação de idade mínima para instrutores (21 anos).
-- Alunos continuam com mínimo de 18 anos.
-- ============================================================

-- Remove assinatura atual (043)
DROP FUNCTION IF EXISTS public.complete_profile_after_signup(uuid,text,text,text,text,date,text,text,text,text,text,integer,text,numeric,numeric,text,text,boolean,boolean,text,integer,text,text,text,text,text,text,text);
...
CREATE OR REPLACE FUNCTION public.complete_profile_after_signup(
  ...
  p_renach               text    DEFAULT NULL,
  p_car_color            text    DEFAULT NULL,
  p_car_plate            text    DEFAULT NULL,
  p_moto_color           text    DEFAULT NULL,
  p_moto_plate           text    DEFAULT NULL
)
RETURNS void
LANGUAGE plpgsql
...
BEGIN
  ...
  -- 3. Validação de idade mínima
  IF p_birthdate IS NOT NULL THEN
    v_age := date_part('year', age(current_date, p_birthdate));
    IF p_role = 'instructor' AND v_age < 21 THEN
      RAISE EXCEPTION 'age_restriction: instrutores devem ter pelo menos 21 anos';
    END IF;
    IF p_role = 'user' AND v_age < 18 THEN
      RAISE EXCEPTION 'age_restriction: alunos devem ter pelo menos 18 anos';
    END IF;
  END IF;

  -- 4. Insere ou atualiza o perfil completo
  ...
  ON CONFLICT (id) DO UPDATE SET
    ...
    moto_color           = EXCLUDED.moto_color,
    moto_plate           = EXCLUDED.moto_plate,
