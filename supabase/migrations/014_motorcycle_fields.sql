-- ============================================================
-- Adiciona campos de moto para instrutores categoria A / A+B.
--
-- Colunas novas:
--   • has_moto     → instrutor possui moto própria (boolean)
--   • moto_model   → marca e modelo da moto
--   • moto_year    → ano da moto
--   • moto_options → quem fornece a moto: 'instructor' | 'student' | 'both'
-- ============================================================

ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS has_moto     boolean DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS moto_model   text    DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS moto_year    integer DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS moto_options text    DEFAULT NULL;

-- Remove a versão anterior da RPC para substituir com os novos parâmetros
DROP FUNCTION IF EXISTS public.complete_profile_after_signup(uuid,text,text,text,text,date,text,text,text,text,text,integer,text,numeric,numeric,text,text,boolean);
