...
import { useInstructorLiveLocation } from '../../hooks/useInstructorLiveLocation';
...
export default function TrackInstructorScreen({ route, navigation }) {
  const { instructorId, instructorName, instructorAvatar, classTime, meetingPoint } = route.params;

  const { user } = useAuth();
  const instructorLoc = useInstructorLiveLocation(instructorId);
  const { location: myLoc } = useCurrentLocation();

  // Enquanto a tela estiver aberta, envia a localização do aluno para o instrutor ver
  useEffect(() => {
    if (!user?.id || !myLoc) return;
    supabase
      .from('profiles')
      .update({ coordinates: { latitude: myLoc.latitude, longitude: myLoc.longitude } })
      .eq('id', user.id)
      .then(() => {});
  }, [user?.id, myLoc?.latitude, myLoc?.longitude]);

  const { distanceKm, etaMin } = useMemo(() => {
    if (!instructorLoc || !myLoc) return { distanceKm: null, etaMin: null };
    const d = haversineKm(
      instructorLoc.latitude, instructorLoc.longitude,
  ...
  const mapCenter = meetingCoords
    ...
    : instructorLoc
      ? { lat: instructorLoc.latitude, lng: instructorLoc.longitude }
      : myLoc
  ...
  const markers = useMemo(() => {
    ...
    if (instructorLoc) {
      m.push({
        id: 'instructor',
        latitude:  instructorLoc.latitude,
        longitude: instructorLoc.longitude,
        label:     instructorName?.split(' ')[0] ?? 'Instrutor',
        color:     '#7C3AED',
        type:      'default',
      });
    }
    ...
    return m;
  }, [instructorLoc, myLoc, meetingCoords, instructorName]);

  const lastSeen = instructorLoc?.updated_at
    ? new Date(instructorLoc.updated_at).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })
    : null;
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Acompanhar instrutor</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.instructorRow}>
          <Avatar uri={instructorAvatar} name={instructorName} size={48} />
          <View style={{ flex: 1 }}>
            <Text style={styles.instructorName}>{instructorName || 'Instrutor'}</Text>
            {classTime ? (
        ...
        {instructorLoc && myLoc ? (
          <View style={styles.etaRow}>
            ...
            <View style={styles.etaBlock}>
              ...
              <View>
                ...
                <Text style={[styles.etaValue, { color: '#7C3AED' }]}>
          ...
          <View style={styles.noLocRow}>
            ...
            <Text style={styles.noLocText}>
              {!instructorLoc
                ? 'O instrutor ainda não iniciou o compartilhamento de localização.'
...
const styles = StyleSheet.create({
  ...
  instructorRow: { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 16 },
  instructorName: { fontSize: 16, fontWeight: '800', color: '#0F172A' },
