-- ============================================================
-- Adiciona cor e placa aos veículos (carro e moto) no perfil.
-- Adiciona car_option à tabela de eventos (veículo utilizado).
-- ============================================================

-- Novos campos em profiles
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS car_color  text DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS car_plate  text DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS moto_color text DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS moto_plate text DEFAULT NULL;

-- car_option nos eventos (qual veículo será usado na aula)
ALTER TABLE public.events
  ADD COLUMN IF NOT EXISTS car_option text DEFAULT 'instructor';

-- Remove versão anterior da RPC (assinatura de 042)
DROP FUNCTION IF EXISTS public.complete_profile_after_signup(uuid,text,text,text,text,date,text,text,text,text,text,integer,text,numeric,numeric,text,text,boolean,boolean,text,integer,text,text,text);
